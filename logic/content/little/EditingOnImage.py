...
############ transparent round ############
def add_transparent_layer(image_path, output_path, bordered_image_height , image_index ,canvas_width=1920, canvas_height=1080,dir_path=None):
    ...
    y_offset = (canvas_height - bordered_image_height) // 2
    ...
    mask_path = f"{dir_path}/extracted_mask{image_index}.png"
    mask = canvas.split()[-1]
    mask = mask.convert("RGBA")
    mask.save(mask_path,format="PNG")
    ## extract mask from image ##
    canvas_without_mask = canvas.convert("RGB")
    canvas_without_mask.save(output_path, format="PNG")
    return mask_path

# Combined function
def process_image_width(image_path, output_path, image_index,target_width=1080,dir_path=None):
    ...
    mask_path = add_transparent_layer(output_path, output_path,bordered_image_height= bordered_image.height,image_index=image_index,dir_path=dir_path)
    return mask_path

# Combined function
def process_image_height(image_path, output_path, image_index,target_height=1080,dir_path=None):
...
# Combined function
def process_video_width(image_path, output_path,target_width=1080):
    # Step 1: Open the image
    image = Image.open(image_path)
    # Step 2: Resize and add borders
    bordered_image = add_borders_and_resize_width(image, target_width=target_width)
...
# Combined function
def process_video_height(image_path, output_path,target_height=1080):
    # Open the image with Pillow
    image = Image.open(image_path)
